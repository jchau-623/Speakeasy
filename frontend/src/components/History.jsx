...
import { addUserFavoriteThunk, deleteUserFavoriteThunk, deleteUserHistoryThunk } from "../store/userReducer";
...
const History = ({ user } ) => {
  ...
  const deleteHistoryFunction = async (item) => {
    try {
      await dispatch(deleteUserHistoryThunk(item.term? item.term : item.idiom));
